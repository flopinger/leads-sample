...
const ManagementChangesPageIntegrated = ({ data }) => {
  ...
  // Extract all management change events from the data
  ...
  const uniqueDates = [...new Set(managementEvents.map(event => event.event_date))].sort().reverse();

  // Filter events
  const filteredEvents = managementEvents.filter(event => {
    const matchesSearch = !searchTerm || 
      ...
      event.details?.company_name?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const totalChanges = filteredEvents.length;
  const uniqueCompanies = new Set(filteredEvents.map(event => event.workshop_id)).size;
  const uniqueCities = new Set(filteredEvents.map(event => event.workshop_city)).size;
  const recentChanges = filteredEvents.filter(event => {
    const eventDate = parseGermanDate(event.event_date);
  ...
  const exportFilteredCompaniesJSON = () => {
    ...
    const exportMetadata = {
      ...
      description: "Gefilterte Unternehmensdaten basierend auf Management-Änderungen"
    };
    ...
    const exportData = {
      ...
      managementEvents: filteredEvents
    };
    ...
    a.href = url;
    a.download = `management_aenderungen_gefiltert_${new Date().toISOString().split('T')[0]}.json`;
    a.click();
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={exportFilteredCompaniesJSON}>
                      ...
                      {t('managementChanges.json')}
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={exportFilteredCompaniesCSV}>
                      <Download className="h-4 w-4 mr-2" />
                      {t('managementChanges.csv')}
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
                    <Link to="/api-docs">
                      <DropdownMenuItem>
                        <Code className="h-4 w-4 mr-2" />
                        {t('managementChanges.api')}
                      </DropdownMenuItem>
                    </Link>
                  </DropdownMenuContent>
                </DropdownMenu>
                        ...
                        {Object.entries(event.details.representatives).map(([key, rep]) => (
                          rep && (
                            <div key={key} className="flex items-center space-x-2">
                              <Users className="h-4 w-4 text-[#005787]" />
                              <span className="text-gray-900">{rep}</span>
                            </div>
                          )
                        ))}
                      </div>
...
export default ManagementChangesPageIntegrated;
